...
const TOTAL_STEPS = 12; // 0-11 (combined birth date + gender into one step)
...
export default function OnboardingScreen({ onComplete, userId }: OnboardingScreenProps) {
  ...
  const [birthDateConfirmed, setBirthDateConfirmed] = useState(false); // Show gender only after confirming birth date
  ...
  const goNext = () => {
    ...
    // Step 3 (birth date + gender) → First confirm birth date, then show gender
    if (step === 3 && !birthDateConfirmed) {
      setBirthDateConfirmed(true);
      setGender(null); // Reset gender selection
      return;
    }
  ...
  const goBack = () => {
    if (step === 3 && birthDateConfirmed) {
      // If we're on step 3 and gender is showing, go back to birth date
  ...
  const canProceed = () => {
    switch (step) {
      ...
      case 3: return birthDateConfirmed ? gender !== null : birthYear !== null; // birth date first, then gender
      case 4: return true; // age range always has defaults
      case 5: return nomadType !== null;
      case 6: return true;
      case 7: return true;
      case 8: return selectedInterests.length >= 1;
      case 9: return lookingFor.length >= 1;
      case 10: return true; // permissions
      case 11: return true; // trip is optional
      default: return true;
    }
  };
  ...
  const ctaLabel = () => {
    ...
    if (step === 10) return t('setup.next'); // permissions
    if (step === 11) return tripCountry && tripArrivalDate ? 'add trip' : t('setup.skip');
    if (step === 6) return homeCountry ? t('setup.next') : t('setup.skip');
    if (step === 7) return currentCity ? t('setup.next') : t('setup.skip');
    return t('setup.next');
  };
  ...
  const renderStep = () => {
    switch (step) {
      ...
      /* ── 3: Birth Date + Gender (combined) ── */
      case 3:
        return (
          <ScrollView showsVerticalScrollIndicator={false}>
            {/* Show birth date picker if not confirmed yet */}
            {!birthDateConfirmed && (
              <>
            ...
            {/* Show Gender options only after birth date is confirmed */}
            {birthDateConfirmed && getAge() >= 18 && (
              <View>
                <Text style={st.question}>{t('setup.genderQuestion')}</Text>
                <Text style={st.subtitle}>{t('setup.genderSub')}</Text>
                  ...
                  {GENDER_OPTIONS.map((g, i) => (
                    <TouchableOpacity key={i} style={[st.optionRow, gender === i && st.optionRowSelected]} onPress={() => setGender(i)} activeOpacity={0.7}>
                      ...
                      <View style={st.optionInfo}>
                        <Text style={st.optionLabel}>{t(`gender.${g.label}`)}</Text>
                      </View>
